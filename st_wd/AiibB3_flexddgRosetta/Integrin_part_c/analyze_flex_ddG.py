# modifying this because I don't want it to check for 'no more batches to process'
# and success line in the rosetta output, bc sometimes I killed them and re-ran 
# them and they skipped the ones that were already run (so it didn't have the
# success line)
#!/usr/bin/python3
...
def analyze_output_folder( output_folder ):
    ...
    df = pd.concat( ddg_scores_dfs ,sort=False)
    ...
    #for score_type in ['mut_dG', 'wt_dG', 'ddG']:
        print( score_type )
        print( '' )
        # sort by the order in my plotting script 
        pickle = df.loc[ df['scored_state'] == score_type ][display_columns]
        pickle = pickle[pickle['score_function_name']=='fa_talaris2014-gam']
        print(pickle)
        #pickle = pickle[pickle['nstruct']==35]
        pickle['case_name'] = pd.Categorical(pickle['case_name'], ['3FCSA71', '3FCSA73', '3FCSA153', '3FCSA155', '3FCSA158', '3FCSA160', '3FCSA174', '3FCSA176', '3FCSA254', '3FCSA313', '3FCSB383', '3FCSB425', '3FCSB434', '3FCSB457', '3FCSB489', '3FCSB495', '3FCSB365'], ordered=True)
        pickle = pickle.sort_values(by='case_name')
        pkl.dump(pickle, open('pickled_out.pkl','wb'))
