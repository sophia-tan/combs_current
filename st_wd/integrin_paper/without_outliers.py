...
def get_correlation(megalist):
    ...
    activation = [activation[x] for x in range(len(activation)) if x != 4 and x!=7 and x!=9 and x!=10 and x!=13] # delete 
    for origcalc in megalist:
        ''' ughhhhhh taking out the funky serine'''
        calc = [origcalc[x] for x in range(len(origcalc)) if x != 4 and x!=7 and x!=9 and x!=10 and x!=13] # delete 

        spearcorr = stats.spearmanr(activation,calc)
        pearscorr = stats.pearsonr(activation,calc)
        #if spearcorr[0] > .5 and pearscorr[0] > .7:
        print(spearcorr[0],pearscorr[0])
        #print(origcalc,'calc')
