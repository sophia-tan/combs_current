...
def rmsd_filter(method,cutoff,score):
    ...
    for targetres, resn in integrin_res.items():
        ...
        if score == 'a':
            ...
            avgclus_for_all_targets.append(sum(np.log10(rawcounts/avgclus)))
        
        if score == 'b':
            #print('method: for each interacting res, take log(num/denom). then, average.')
            rawcounts_for_all_targets.append(np.mean(rawcounts))
            avgclus_for_all_targets.append(np.mean(np.log10(rawcounts/avgclus)))

        if score == 'c':
            #print('method: sum across all num and denom for each interacting res. then, take the log')
            rawcounts_for_all_targets.append(sum(rawcounts))
            avgclus_for_all_targets.append(np.log10(sum(rawcounts)/sum(avgclus)))

        if score == 'd':
            #print('method: avg across all num and denom for each interacting res. then, take the log')
            rawcounts_for_all_targets.append(np.mean(rawcounts))
            avgclus_for_all_targets.append(np.log10(np.mean(rawcounts)/np.mean(avgclus)))

    return [rawcounts_for_all_targets, avgclus_for_all_targets]
