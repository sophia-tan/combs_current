...
def score_interaction_and_dump(
    ...
    if ifgtype[1] != ['N', 'CA', 'C'] and ifgtype[1] != ['CA', 'C', 'O']:
        ...
        get_NN(lookupatoms_to_clus, num_atoms, rmsds, query, cutoff)


def get_NN(lookupatoms_to_clus, num_atoms, rmsds, query, cutoff):
    flat = [x.flatten() for x in lookupatoms_to_clus]
    euclid_dist = cutoff * np.sqrt(num_atoms)
    print(euclid_dist, 'euclid dist', num_atoms)
    ...
    # compare to see how many rmsd < .5 or w/e the cutoff is
    rmsds = rmsds[1:]
