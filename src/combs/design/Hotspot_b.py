...
def make_new_interactamer_coords(pdb, comb, origin_atom, plane_atom1, plane_atom2):
    ...
    vdm_coords = np.array([new_origin_coords, new_plane_atom1_coords, new_plane_atom2_coords])
    ...
    return [vdm_coords]*len(ifg_coords), ifg_coords, pdbcopies


# def make_new_vdm_coords(pdb, comb):
#     return make_new_interactamer_coords(pdb, comb, 'CA', 'N', 'C')
...
# how to flatten the coord arrays: np.array([coords.flatten() for coords in vdms])

def make_interactamers(path_to_csv, outpath, comb, seq_distance=20):
    if outpath[-1] != '/':
        outpath += '/'

    an = Analyze(path_to_csv)
    dist_vdms = an.get_distant_vdms(seq_distance=seq_distance)
    resn_ints = set(dist_vdms.groupby('resname_vdm').groups).intersection(set(interactamer_atoms.keys()))
    # resn_ints.add('BB_CO')
    # resn_ints.add('BB_NH')
    for resn in resn_ints:
        # try:
        #     os.makedirs(outpath + resn)
        # except:
        #     pass
        ...
        try:
            ...
            pass
        interactamer_output = []
        vdms = parse_interactamers_sc(dist_vdms, an, resn)
        print('Making interactamers for ' + resn + '...')
        for vdm in vdms:
            try:
                ifg_count = int(repr(vdm).split('_')[1])
                vdm_count = int(repr(vdm).split('_')[3])
                vdm_coords, ifg_coords, pdbs = make_new_interactamer_coords(vdm, comb, interactamer_atoms[resn][0],
                                                                           interactamer_atoms[resn][1],
                                                                           interactamer_atoms[resn][2])
                for i, vc, ic, pdb in zip(range(len(vdm_coords)), vdm_coords, ifg_coords, pdbs):
                    interactamer_output.append([ifg_count, vdm_count, 1, i+1, vc, ic, resn])
                    string = repr(pdb).split()[1].split('_')
                    pr.writePDB(pdb_path + '_'.join(string[:-1]) + '_vFlip_1_' + 'iFlip_' + str(i+1)
                                + '_' + string[-1] + '_oriented.pdb.gz', pdb)

                if resn in flip_residues:
                    vdm_coords, ifg_coords, pdbs = make_new_interactamer_coords(vdm, comb, interactamer_atoms[resn][0],
                                                                              interactamer_atoms[resn][2],
                                                                              interactamer_atoms[resn][1])
                    for i, vc, ic, pdb in zip(range(len(vdm_coords)), vdm_coords, ifg_coords, pdbs):
                        interactamer_output.append([ifg_count, vdm_count, 2, i+1, vc, ic, resn])
                        pr.writePDB(pdb_path + '_'.join(string[:-1]) + '_vFlip_2_' + 'iFlip_' + str(i+1)
                                    + '_' + string[-1] + '_oriented.pdb.gz', pdb)
            except Exception:
                traceback.print_exc()
        # output format = [ifg_count, vdm_count, vdm_flip, ifg_flip, vdm_coords, ifg_coords]
        with open(picklepath + resn + '_interactamers.pickle', 'wb') as f:
            pickle.dump(np.array(interactamer_output, dtype=object), f)

    for resn in ['BB_CO', 'BB_NH']:
        if resn == 'BB_CO':
            pdb_path = outpath + 'pdbs/' + resn + '/O_not_N/'
            try:
                os.makedirs(pdb_path)
            except:
                pass
            ...
            try:
                ...
                pass
            interactamer_output = []
            vdms = parse_interactamers_bb_single_atom(dist_vdms, an, 'O', 'N')
            print('Making interactamers for ' + resn + '...')
            for vdm in vdms:
                try:
                    ifg_count = int(repr(vdm).split('_')[1])
                    ...
                    vdm_coords, ifg_coords, pdbs = make_new_interactamer_coords(vdm, comb, interactamer_atoms[resn][0],
                                                                                ...
                                                                                interactamer_atoms[resn][2])
                    for i, vc, ic, pdb in zip(range(len(vdm_coords)), vdm_coords, ifg_coords, pdbs):
                        interactamer_output.append([ifg_count, vdm_count, 1, i + 1, vc, ic, resn + '_O_not_N'])
                        string = repr(pdb).split()[1].split('_')
                        pr.writePDB(pdb_path + '_'.join(string[:-1]) + '_vFlip_1_' + 'iFlip_' + str(i + 1)
                                    + '_' + string[-1] + '_oriented.pdb.gz', pdb)
                    ...
                    if resn in flip_residues:
                        vdm_coords, ifg_coords, pdbs = make_new_interactamer_coords(vdm, comb, interactamer_atoms[resn][0],
                                                                                    ...
                                                                                    interactamer_atoms[resn][1])
                        for i, vc, ic, pdb in zip(range(len(vdm_coords)), vdm_coords, ifg_coords, pdbs):
                            interactamer_output.append([ifg_count, vdm_count, 2, i + 1, vc, ic, resn + '_O_not_N'])
                            pr.writePDB(pdb_path + '_'.join(string[:-1]) + '_vFlip_2_' + 'iFlip_' + str(i + 1)
                                        + '_' + string[-1] + '_oriented.pdb.gz', pdb)
                except Exception:
                    traceback.print_exc()
            # output format = [ifg_count, vdm_count, vdm_flip, ifg_flip, vdm_coords, ifg_coords]
            with open(picklepath + resn + '_O_not_N_interactamers.pickle', 'wb') as f:
                pickle.dump(np.array(interactamer_output, dtype=object), f)

        if resn == 'BB_NH':
            pdb_path = outpath + 'pdbs/' + resn + '/N_not_O/'
            try:
                os.makedirs(pdb_path)
            except:
                pass
            ...
            try:
                ...
                pass
            interactamer_output = []
            vdms = parse_interactamers_bb_single_atom(dist_vdms, an, 'N', 'O')
            print('Making interactamers for ' + resn + '...')
            for vdm in vdms:
                try:
                    ifg_count = int(repr(vdm).split('_')[1])
                    vdm_count = int(repr(vdm).split('_')[3])
                    vdm_coords, ifg_coords, pdbs = make_new_interactamer_coords(vdm, comb,
                                                                                interactamer_atoms[resn][0],
                                                                                interactamer_atoms[resn][1],
                                                                                interactamer_atoms[resn][2])
                    for i, vc, ic, pdb in zip(range(len(vdm_coords)), vdm_coords, ifg_coords, pdbs):
                        interactamer_output.append([ifg_count, vdm_count, 1, i + 1, vc, ic, resn + '_N_not_O'])
                        string = repr(pdb).split()[1].split('_')
                        pr.writePDB(pdb_path + '_'.join(string[:-1]) + '_vFlip_1_' + 'iFlip_' + str(i + 1)
                                    + '_' + string[-1] + '_oriented.pdb.gz', pdb)
                    ...
                    if resn in flip_residues:
                        vdm_coords, ifg_coords, pdbs = make_new_interactamer_coords(vdm, comb,
                                                                                    ...
                                                                                    interactamer_atoms[resn][1])
                        for i, vc, ic, pdb in zip(range(len(vdm_coords)), vdm_coords, ifg_coords, pdbs):
                            interactamer_output.append([ifg_count, vdm_count, 2, i + 1, vc, ic, resn + '_N_not_O'])
                            pr.writePDB(pdb_path + '_'.join(string[:-1]) + '_vFlip_2_' + 'iFlip_' + str(i + 1)
                                        + '_' + string[-1] + '_oriented.pdb.gz', pdb)
                except Exception:
                    traceback.print_exc()
            # output format = [ifg_count, vdm_count, vdm_flip, ifg_flip, vdm_coords, ifg_coords]
            with open(picklepath + resn + '_N_not_O_interactamers.pickle', 'wb') as f:
                pickle.dump(np.array(interactamer_output, dtype=object), f)
        ...
        for phi_low, phi_high in zip(range(-180, 180, 36), range(-144, 216, 36)):
            for psi_low, psi_high in zip(range(-180, 180, 36), range(-144, 216, 36)):
                pdb_path = outpath + 'pdbs/' + resn + '/' + str(phi_low) + '_' + str(phi_high) + '_' + str(psi_low) \
                     + '_' + str(psi_high) + '/'
                try:
                    os.makedirs(pdb_path)
                except:
                ...
                dist_vdms_sub = dist_vdms[dist_vdms.apply(phi_psi_select, axis=1)]
                interactamer_output = []
                vdms = parse_interactamers_bb_both_atoms(dist_vdms_sub, an, 'N', 'O')
                print('Making interactamers for ' + resn + '...')
                resn_string = resn + '_' + str(phi_low) + '_' + str(phi_high) + '_' + str(psi_low) \
                     + '_' + str(psi_high)
                ...
                for vdm in vdms:
                    try:
                        ...
                        vdm_coords, ifg_coords, pdbs = make_new_interactamer_coords(vdm, comb, interactamer_atoms[resn][0],
                                                                                   ...
                                                                                   interactamer_atoms[resn][2])
                        for i, vc, ic, pdb in zip(range(len(vdm_coords)), vdm_coords, ifg_coords, pdbs):
                            interactamer_output.append([ifg_count, vdm_count, 1, i+1, vc, ic, resn_string])
                            string = repr(pdb).split()[1].split('_')
                            pr.writePDB(pdb_path + '_'.join(string[:-1]) + '_vFlip_1_' + 'iFlip_' + str(i+1)
                                        + '_' + string[-1] + '_oriented.pdb.gz', pdb)
                        ...
                        if resn in flip_residues:
                            vdm_coords, ifg_coords, pdbs = make_new_interactamer_coords(vdm, comb, interactamer_atoms[resn][0],
                                                                                      ...
                                                                                      interactamer_atoms[resn][1])
                            for i, vc, ic, pdb in zip(range(len(vdm_coords)), vdm_coords, ifg_coords, pdbs):
                                interactamer_output.append([ifg_count, vdm_count, 2, i+1, vc, ic, resn_string])
                                pr.writePDB(pdb_path + '_'.join(string[:-1]) + '_vFlip_2_' + 'iFlip_' + str(i+1)
                                            + '_' + string[-1] + '_oriented.pdb.gz', pdb)
                    except Exception:
                        traceback.print_exc()
                # output format = [ifg_count, vdm_count, vdm_flip, ifg_flip, vdm_coords, ifg_coords]
                with open(picklepath + resn + '_' + str(phi_low) + '_' + str(phi_high) + '_' + str(psi_low)
                          + '_' + str(psi_high) + '_interactamers.pickle', 'wb') as f:
                    pickle.dump(np.array(interactamer_output, dtype=object), f)


def make_rel_vdms(path_to_csv, outpath, comb, seq_distance=10):
    if outpath[-1] != '/':
        outpath += '/'
    ...
    for resn in resn_ints:
        ...
        try:
            ...
            pass
        interactamer_output = []
        vdms = parse_interactamers_aa(dist_vdms, an, resn) # will need to change if considering phi/psi
        print('Making relative vdMs for ' + resn + '...')
        for vdm in vdms:
            try:
                ifg_count = int(repr(vdm).split('_')[1])
                vdm_count = int(repr(vdm).split('_')[3])
                # vdm_coords, ifg_coords, pdbs = make_new_vdm_coords(vdm, comb)
                vdm_coords, ifg_coords, pdbs = make_new_interactamer_coords(vdm, comb, 'CA', 'N', 'C')
                for i, vc, ic, pdb in zip(range(len(vdm_coords)), vdm_coords, ifg_coords, pdbs):
                    interactamer_output.append([ifg_count, vdm_count, 1, i+1, vc, ic, resn])
                    string = repr(pdb).split()[1].split('_')
                    pr.writePDB(pdb_path + '_'.join(string[:-1]) + '_vFlip_1_' + 'iFlip_' + str(i+1)
                ...
                # if resn in flip_residues:
                #     vdm_coords, ifg_coords, pdbs = make_new_vdm_coords(vdm, comb, interactamer_atoms[resn][0],
                #                                                               interactamer_atoms[resn][2],
                #                                                               interactamer_atoms[resn][1])
                #     for i, vc, ic, pdb in zip(range(len(vdm_coords)), vdm_coords, ifg_coords, pdbs):
                #         interactamer_output.append([ifg_count, vdm_count, 2, i+1, vc, ic, resn])
    ...
    #     pdb_path = outpath + 'pdbs/' + resn + '/'
    #     try:
    #         os.makedirs(pdb_path + 'NH')
    #         os.makedirs(pdb_path + 'CO')
    #     except:
    #         pass
    #     picklepath = outpath + 'pickle/'
    #     try:
    #         os.makedirs(picklepath + 'NH')
    #         os.makedirs(picklepath + 'CO')
    #     except:
    #         pass
    #     interactamer_output = []
    #     vdms = parse_interactamers_aa(dist_vdms, an, resn) # will need to change if considering phi/psi
    ...
    #             vdm_count = int(repr(vdm).split('_')[3])
    #             # vdm_coords, ifg_coords, pdbs = make_new_vdm_coords(vdm, comb)
    #             vdm_coords_nh, ifg_coords_nh, pdbs_nh = make_new_interactamer_coords(vdm, comb, 'N', 'H', 'CA')
    #             for i, vc, ic, pdb in zip(range(len(vdm_coords_nh)), vdm_coords_nh, ifg_coords_nh, pdbs_nh):
    #                 interactamer_output.append([ifg_count, vdm_count, 1, i+1, vc, ic, resn, 'NH'])
    #                 string = repr(pdb).split()[1].split('_')
    #                 pr.writePDB(pdb_path + 'NH/' + '_'.join(string[:-1]) + '_vFlip_1_' + 'iFlip_' + str(i+1)
    #                             + '_' + string[-1] + '_oriented.pdb.gz', pdb)
    #
    #             # if resn in flip_residues:
    #             #     vdm_coords, ifg_coords, pdbs = make_new_vdm_coords(vdm, comb, interactamer_atoms[resn][0],
    ...
    #             traceback.print_exc()
    #     # output format = [ifg_count, vdm_count, vdm_flip, ifg_flip, vdm_coords, ifg_coords]
    #     with open(picklepath + 'NH/' + resn + '_rel_vdms.pickle', 'wb') as f:
    #         pickle.dump(np.array(interactamer_output, dtype=object), f)
    #
    #     for vdm in vdms:
    #         try:
    ...
    #             vdm_count = int(repr(vdm).split('_')[3])
    #             # vdm_coords, ifg_coords, pdbs = make_new_vdm_coords(vdm, comb)
    #             vdm_coords_co, ifg_coords_co, pdbs_co = make_new_interactamer_coords(vdm, comb, 'C', 'O', 'CA')
    #             for i, vc, ic, pdb in zip(range(len(vdm_coords_co)), vdm_coords_co, ifg_coords_co, pdbs_co):
    #                 interactamer_output.append([ifg_count, vdm_count, 1, i + 1, vc, ic, resn, 'CO'])
    #                 string = repr(pdb).split()[1].split('_')
    #                 pr.writePDB(pdb_path + 'CO/' + '_'.join(string[:-1]) + '_vFlip_1_' + 'iFlip_' + str(i + 1)
    #                             + '_' + string[-1] + '_oriented.pdb.gz', pdb)
    #
    #                 # if resn in flip_residues:
    #                 #     vdm_coords, ifg_coords, pdbs = make_new_vdm_coords(vdm, comb, interactamer_atoms[resn][0],
    ...
    #             traceback.print_exc()
    #     # output format = [ifg_count, vdm_count, vdm_flip, ifg_flip, vdm_coords, ifg_coords]
    #     with open(picklepath + 'CO/' + resn + '_rel_vdms.pickle', 'wb') as f:
    #         pickle.dump(np.array(interactamer_output, dtype=object), f)
...
def parse_interactamers_sc(df, an, resn):
    #use dist_vdms as dataframe (df)
    df = df[df['resname_vdm'] == resn]
    int_df = df[df.apply(lambda row: any(name in interactamer_atoms[resn]
                                                for name in row['atom_names_vdm'].split()), axis=1)]
    return an.parse_vdms(int_df)
...
def parse_interactamers_bb_single_atom(df, an, atom_name, not_atom_name):
    #use dist_vdms as dataframe (df)
    def sing_atom(row):
        if atom_name in row['atom_names_vdm'].split():
            if not_atom_name not in row['atom_names_vdm'].split():
                return True
            else:
                return False
        else:
    ...
    int_df = df[df.apply(sing_atom, axis=1)]
    return an.parse_vdms(int_df)


def parse_interactamers_bb_both_atoms(df, an, atom_name1, atom_name2):
    # use dist_vdms as dataframe (df)
    def sing_atom(row):
        if atom_name1 in row['atom_names_vdm'].split():
            if atom_name2 in row['atom_names_vdm'].split():
                return True
            else:
                return False
        else:
...
def relative_interactamer_coords(resn, roi, vdm, comb, flips=None):
    roi_sel = roi.select('name ' + ' '.join(interactamer_atoms[resn]))
    vdm_sel = vdm.select('resnum 10 and chain X and name ' + ' '.join(interactamer_atoms[resn]))
    pdbcopy = superpose(vdm, vdm_sel, roi_sel, flips=flips)
    ifg_sel = make_ifg_sele(pdbcopy, comb)
    ifg_coords = ifg_sel.getCoords()
    return ifg_coords, pdbcopy


def make_relative_interactamer_coords(roi, vdm, poi_coords, comb, resn=None, clash_cutoff=2.5):
    resn = resn or roi.getResnames()[0]
    ifg_coords_all = []
    pdb_sup_all = []
    # make change to code here concerning iFG atom flips
    ifg_coords, pdb_sup = relative_interactamer_coords(resn, roi, vdm, comb)
    if calc_clash_score(poi_coords, ifg_coords, clash_cutoff=clash_cutoff) < 1:
        ifg_coords = [y for x in ifg_coords for y in x]
        ifg_coords_all.append(ifg_coords)
        pdb_sup_all.append(pdb_sup)
    if resn in flip_names.keys():
        ifg_coords, pdb_sup = relative_interactamer_coords(resn, roi, vdm, comb, flips=flip_names[resn])
        if calc_clash_score(poi_coords, ifg_coords, clash_cutoff=clash_cutoff) < 1:
            ifg_coords = [y for x in ifg_coords for y in x]
            ifg_coords_all.append(ifg_coords)
            pdb_sup_all.append(pdb_sup)
    return ifg_coords_all, pdb_sup_all
...
def load_interactamers(path_to_pickle):
    interactamers = {}
    interactamers['BB_CO'] = {}
    interactamers['BB_NH'] = {}
    for file in [file for file in os.listdir(path_to_pickle) if file[0] is not '.']:
        ...
        if splitfile[0] == 'BB':
            ...
            with open(path_to_pickle + '/' + file, 'rb') as infile:
                interactamers[file.split('interactamers')[0][:-1]] = pickle.load(infile)
