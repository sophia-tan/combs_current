...
class IntFG:
    ...
    def __init__(self, parsed_pdb, comb):
        ...
        self.ifg_frag = parsed_pdb.prody_pdb.select('chain ' + self.chid + ' and resnum `' + str(np.min(self.resnum)-1)
    ...
    # @staticmethod
    # def select_contacts(parsed_pdb, resinds):
    #     sel = parsed_pdb.select('resindex ' + str(resinds[0]))
    #     for resind in resinds[1:]:
    #         sel = sel | parsed_pdb.select('resindex ' + str(resind))
    #     return sel
    @staticmethod
    ...
    def get_hbonds(self, parsed_pdb):
    ...
    def get_ca_hbonds(self, parsed_pdb):
    ...
    def get_rotamer(self, parsed_pdb):
        _rotamers = [calc_rotamer(parsed_pdb.prody_pdb, rn, self.chid) for rn in self.resnum]
        newrots = []
        for rot in _rotamers:
            newrot = []
            for item in rot:
                newitem = []
                for chi in item:
                    newitem.append('{0:.2f}'.format(float(chi)))
                newrot.append('(' + ' '.join(newitem) + ')')
            newrots.append('[' + ' '.join(newrot) + ']')
        self.rotamer = ' '.join(newrots)

    def get_dssp_sasa(self):
        ri, _ind = np.unique(self.sele.getResindices(), return_index=True)
