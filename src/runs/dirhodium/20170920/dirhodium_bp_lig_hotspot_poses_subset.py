...
indir = '/Users/npolizzi/Projects/combs/results/dirhodium/20170919/'
...
relvdm_carboxylate1.hotspot_subgraphs = list(
    sorted((comp for comp in nx.connected_component_subgraphs(relvdm_carboxylate1.hotspot_graph) if len(comp) > 4),
           key=len, reverse=True))

relvdm_carboxylate2.hotspot_subgraphs = list(
    sorted((comp for comp in nx.connected_component_subgraphs(relvdm_carboxylate2.hotspot_graph) if len(comp) > 4),
           key=len, reverse=True))

relvdm_carboxylate3.hotspot_subgraphs = list(
    sorted((comp for comp in nx.connected_component_subgraphs(relvdm_carboxylate3.hotspot_graph) if len(comp) > 4),
           key=len, reverse=True))

relvdm_carboxylate4.hotspot_subgraphs = list(
    sorted((comp for comp in nx.connected_component_subgraphs(relvdm_carboxylate4.hotspot_graph) if len(comp) > 4),
           key=len, reverse=True))
...
sample.find_ligand_cst_hotspots(tol=.4)
