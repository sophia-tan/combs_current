...
outdir = '/Users/npolizzi/Projects/combs/results/apixaban/20171024/backbone/'
try:
    os.makedirs(outdir)
except:
    pass
...
mems, cents = combs.Cluster.greedy_cluster_gt(adj_mat, gt=20)
all_resnum_chid = functools.reduce(lambda a, b: np.vstack((a, b)),
                                   [np.array([tuple(key)]*len(val), dtype=object)
...
relvdms = [relvdm_backbone, relvdm_carboxamide, relvdm_backbone2]
sample.relvdms = {}
...
sample.fit_lig_to_cluster_densities(relvdms, order=['carboxamide', 'backbone', 'backbone2'], num_proc=8)
