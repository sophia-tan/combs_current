...
sample = combs.Sample()
poi_path = '/Users/npolizzi/Projects/combs/src/runs/apixaban/20171114/'

# site_path = '/Users/npolizzi/Downloads/scPDB_2013/sc_gt80_le300_MWgt400_sitele35res_sites_pdbs/'

sample.poi = pr.parsePDB(poi_path + '00001.213347213288.allbb_rottrans.pdb')
# site = pr.parsePDB(site_path + '00001.bdc94b8f1d9d.allbb.pdb')
...
resis = [7,10,11,13,14,16,17,18,20,21,24]
chids = ['A']*len(resis)
chids.extend(['B']*len(resis))
chids.extend(['C']*len(resis))
chids.extend(['D']*len(resis))

sample.bs_residues = list(zip(resis*4, chids))
print(sample.bs_residues)
sample.set_rois()
sample.set_rois_phipsi()
sample.set_poi_clash_coords()
sample.set_roi_bb_coords()
sample.ligand_conformers = [pr.parsePDB(pdb_path + 'AP_0001.pdb')]
...
# mems, cents = combs.Cluster.greedy_cluster_pc(adj_mat, pc=0.75)
...
# outdir = '/Users/npolizzi/Projects/combs/results/apixaban/20171114/00001.213347213288.allbb/backbone/'
# try:
#     os.makedirs(outdir)
# except:
#     pass
...
# mems, cents = combs.Cluster.greedy_cluster_pc(adj_mat, pc=0.9)
# all_resnum_chid = functools.reduce(lambda a, b: np.vstack((a, b)),
#                                    [np.array([tuple(key)]*len(val), dtype=object)
...
#
# # sample.poi = pr.parsePDB(poi_path + '00001.bdc94b8f1d9d.allbb.pdb')
#
...
#     pickle.dump(sample, outfile)
#
# sample.poi = pr.parsePDB(poi_path + '00001.213347213288.allbb.pdb')